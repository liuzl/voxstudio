...
**Table 2**　Gate records on built-in MacBook Pro speakers/microphone with real speech stimuli. Silero is the default detector everywhere as of v1.2 (WASM SIMD in source and compiled builds; native ONNX is an optional explicit measurement backend — §6); for non-explicit VAD selection, the equally-certified energy detector remains as the loud fallback should the selected Silero backend fail.

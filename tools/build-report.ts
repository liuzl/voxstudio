...
html = html.replace(
  /<!--MERMAID-(\d+)-->\s*(<p><strong>图 \d+<\/strong>[\s\S]*?<\/p>)?/g,
  (_match, index: string, caption?: string) => {
    ...
    const cap = caption
      ? `<figcaption>${caption.replace(/^<p>/, "").replace(/<\/p>$/, "").replace(/<strong>(图 \d+)<\/strong>/, "<b>$1</b>")}</figcaption>`
      : "";
    return `<figure class="figure"><div class="frame">${media}` +
      `<details class="figsrc"><summary>图源（mermaid）</summary><pre>${escapeHtml(mermaids[figure] ?? "")}</pre></details>` +
      `</div>${cap}</figure>`;
  },
);
...
html = html.replace(
  /<blockquote>\s*<p><strong>(教训[^<]*)<\/strong>([\s\S]*?)<\/p>\s*<\/blockquote>/g,
...
html = html.replace(/<h2>([\s\S]*?)<\/h2>/g, (_match, inner: string) => {
  ...
  const appendix = /^(附录 [A-Z])　?(.*)$/.exec(text);
  let id: string;
  let no: string;
  let label: string;
  if (numbered) {
  ...
  id = text === "摘要" ? "abstract" : text === "参考文献" ? "references" : `sec-${toc.length}`;
  tocPush(toc, id, "", text);
...
const tocHtml = `<nav aria-label="目录"><h2 style="border:0; padding-top:0; margin-top:1.4rem;">目录</h2><ul class="toc">` +
...
html = html.replace(/(<p>关键词[\s\S]*?<\/p>)/, `$1\n${tocHtml}`);

function escapeHtml(text: string): string {
  return text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

const title = /^# (.+)$/m.exec(md)?.[1]?.replace(/\*/g, "") ?? "VoxStudio 技术报告";
...
<body>
<main class="paper">
<p class="kicker">技术报告 · Technical Report</p>
${html}
</main>
</body>
</html>
